...
const stateInsertTarget = `  const [isCourierBookingOpen, setIsCourierBookingOpen] = useState(false);`;
const stateInsertReplacement = `  const [isCourierBookingOpen, setIsCourierBookingOpen] = useState(false);
  const [autoBookingResult, setAutoBookingResult] = useState<any | null>(null);`;
content = content.replace(stateInsertTarget, stateInsertReplacement);

const autoBookingFetchTarget = `                    const data = await response.json();
...
const autoBookingFetchReplacement = `                    const data = await response.json();
...
content = content.replace(autoBookingFetchTarget, autoBookingFetchReplacement);

const autoBookingModalBodyTarget = `            <div className="p-5 space-y-4 max-h-[70vh] overflow-y-auto">
              <div>`;
const autoBookingModalBodyReplacement = `            <div className="p-5 space-y-4 max-h-[70vh] overflow-y-auto">
              {!autoBookingResult ? (
                <>
              <div>`;
content = content.replace(autoBookingModalBodyTarget, autoBookingModalBodyReplacement);

const autoBookingModalFooterTarget = `              </button>
            </div>
          </div>`;
const autoBookingModalFooterReplacement = `              </button>
            </div>
            </>
            ) : (
...
content = content.replace(autoBookingModalFooterTarget, autoBookingModalFooterReplacement);

// Fix backdrop click issue
content = content.replace(`onClick={() => setIsCourierBookingOpen(false)}`, `onClick={() => { setIsCourierBookingOpen(false); setAutoBookingResult(null); }}`);
content = content.replace(`onClick={() => setIsCourierBookingOpen(false)}`, `onClick={() => { setIsCourierBookingOpen(false); setAutoBookingResult(null); }}`);
content = content.replace(`onClick={() => setIsCourierBookingOpen(false)}`, `onClick={() => { setIsCourierBookingOpen(false); setAutoBookingResult(null); }}`);

fs.writeFileSync(file, content);
console.log('AdminDashboard Auto Booking Patched Successfully');
